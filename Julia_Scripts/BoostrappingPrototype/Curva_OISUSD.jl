...
##################################load Data#############################################
#Tasas:
path = "C:\\Users\\jaime.valenzuela\\Documents\\Julia_Scripts\\BoostrappingPrototype\\Datos\\bidaskOIS_20200527.csv"
dataMktRate = CSV.read(path)
rates = dataMktRate[:, filter(x -> (x in [:tenor,:typeRate ,:bid]), names(dataMktRate))]
rates.mid = rates.bid / 100
#Definiciones/Datos
